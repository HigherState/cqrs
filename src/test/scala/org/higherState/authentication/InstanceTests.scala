...
class InstanceTests extends FunSuite with Matchers with ScalaFutures with BeforeAndAfter {
  ...
  test("Simple service, not designed to handle concurrency") {
    ...
    val ch = new AuthenticationCommandHandler[Identity, Valid](10, testHashRepository){}
    val qe = new AuthenticationQueryExecutor[Identity, Valid](testHashRepository){}
    val testAuthenticationService =
      AuthenticationService(Dispatcher(ch.asInstanceOf[CommandHandler[Valid, Command]], qe.asInstanceOf[QueryExecutor[Valid, QueryParameters]]))
